...
// Un service worker su gbprof.it non deve cancellare cache appartenenti ad altre PWA.
const workers = files.filter(file => /(?:^|\/)(?:sw|service-worker)\.js$/i.test(file.replaceAll(path.sep,"/")));
for (const file of workers) {
  const rel = path.relative(root,file), s = fs.readFileSync(file,"utf8");
  const activateAt = s.search(/addEventListener\(\s*["']activate["']/i);
  if (activateAt < 0) continue;
  const fetchAt = s.slice(activateAt + 1).search(/addEventListener\(\s*["']fetch["']/i);
  const activate = fetchAt < 0 ? s.slice(activateAt) : s.slice(activateAt, activateAt + 1 + fetchAt);
  if (/caches\.keys\s*\(/i.test(activate) && /caches\.delete\s*\(/i.test(activate) && !/startsWith\s*\(/i.test(activate)) {
    failures.push(`${rel}: pulizia cache potenzialmente globale; manca un filtro per il prefisso della PWA`);
  }
}

const lines=["# Audit automatico PWA","",`HTML: ${html.length} · manifest: ${manifests.length} · service worker: ${workers.length}`,`Errori automatici: ${failures.length} · avvisi: ${warnings.length}`,"","## Errori rilevati",...(failures.length?failures.map(x=>`- ${x}`):["- Nessun errore rilevato dai controlli automatici."]),"","## Avvisi",...(warnings.length?warnings.map(x=>`- ${x}`):["- Nessun avviso."]),"","_Questo audit automatico non sostituisce la verifica manuale con tastiera, screen reader e dispositivi reali._",""];
